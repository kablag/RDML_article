...
RDML data standard contains the primary raw data acquired by the machine, 
...
pipelines. Additionally, there is a need for a \textit{de novo} creation of 
RDML data.  It is especially of importance for systems, that do not support 
the RDML format. This includes commercial systems 
and experimental systems (e.g.,  point-of-care devices). 
>>>>>>> 7435d28e75d5e1380941b7107182254d4371b8c3
...
work with the RDML format, there is a need make RDML available for the increasingly 
...
Researchers can no longer collect data in a single laboratory and are often 
are coerced to work with many data sets. Adaptable data management - aka. adaptive 
...
\cite{millard_adaptive_2011}. Since R offers also HDF5 interface \cite{Fischer_HDF5},
the \textit{RDML} package facilates not only data storage and analysis, but also 
the higher level of the experimental data managnent.
